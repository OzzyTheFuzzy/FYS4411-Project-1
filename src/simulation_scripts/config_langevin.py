...
omega_z    = None    # for elliptical trap set omega_z = beta, for spherical trap set omega_z = 
...
# this is not the true dt, but rather a scale factor for the Langevin sampler, which internally adapts the step size per alpha  
dt_array        = np.array([0.001, 0.01, 0.1, 0.5, 1.0, 2.0])  # scale goes as dt 
backend         = "torch"
batch_size      = 200
detailed        = True
