...
def find_E_langevin():
    ...
    return mean_energies, accept_array



mean_energies, accept_array = find_E_langevin()
for dt, acc, energy in zip(config_langevin.dt_array, accept_array, mean_energies):
    print(f"dt = {dt*np.sqrt(2):.4f} → acceptance = {acc:.4f}  →  energy = {energy:.4f}")

plt.scatter(config_langevin.dt_array*np.sqrt(2), mean_energies, label="Energy")
plt.xlabel("dt")
plt.ylabel("Energy")
plt.title(f"E(dt)")
#plt.grid("True")
